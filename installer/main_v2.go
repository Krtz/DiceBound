...
func updateShortcuts(c Config, game, destinations string) error {
	...
	script := fmt.Sprintf(`$ErrorActionPreference='Stop';$w=New-Object -ComObject WScript.Shell;$target=(Get-Item -LiteralPath '%s').FullName;$workingDirectory='%s';$icon='%s,0';%s;function Set-Link($p){$parent=Split-Path -Parent $p;if(-not (Test-Path -LiteralPath $parent -PathType Container)){New-Item -ItemType Directory -Path $parent -Force|Out-Null};$x=$w.CreateShortcut($p);$x.TargetPath=$target;$x.WorkingDirectory=$workingDirectory;$x.IconLocation=$icon;$x.Save();$check=$w.CreateShortcut($p);if($check.TargetPath -ne $target){throw "Shortcut target verification failed for $p. Expected '$target', got '$($check.TargetPath)'."}};function Remove-Link($p){if(Test-Path -LiteralPath $p -PathType Leaf){Remove-Item -LiteralPath $p -Force;if(Test-Path -LiteralPath $p){throw "Could not remove shortcut '$p'."}}};if(%s){Set-Link $d}else{Remove-Link $d};if(%s){Set-Link $s}else{Remove-Link $s}`, psq(launcher), psq(c.InstallDir), psq(game), destinations, psbool(c.Desktop), psbool(c.StartMenu))
