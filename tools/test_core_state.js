...
const persisted={
  ...
  settings:{masterVolume:2,soundPack:"custom",muted:true},
...
assert.equal(defaults.settings.characterLayout,"modern","new careers must default to Modern Character layout");
const legacyDefaultSnapshot=JSON.parse(JSON.stringify(defaults));
delete legacyDefaultSnapshot.settings.characterLayout;
delete legacyDefaultSnapshot.heirloomStorage;
delete legacyDefaultSnapshot.echoCrucible;
const serialized=JSON.stringify(legacyDefaultSnapshot);
assert.equal(Buffer.byteLength(serialized),864,"default career byte snapshot drifted outside intentional Character-layout/Vault/Crucible schema additions");
assert.equal(crypto.createHash("sha256").update(serialized).digest("hex"),"d22ff6b1fe8d7021faede28d594d512650b4049f95098109dca984ff14bbcf2f","default career data drifted outside intentional Character-layout/Vault/Crucible schema additions");
assert.deepEqual([...defaults.heirloomStorage],[],"fresh careers must start with an empty Heirloom Vault");
assert.deepEqual(JSON.parse(JSON.stringify(defaults.echoCrucible)),{learnedEffectIds:[],selectedEffectId:null,moonMetal:0},"fresh careers must start with an empty Echo Crucible");
...
delete legacyLive.settings.characterLayout;
delete legacyLive.heirloomStorage;
delete legacyLive.echoCrucible;
const legacyLiveSerialized=JSON.stringify(legacyLive);
assert.equal(Buffer.byteLength(legacyLiveSerialized),2056,"pre-Math live default-career byte snapshot drifted");
assert.equal(crypto.createHash("sha256").update(legacyLiveSerialized).digest("hex"),"c8ee60eccc857138f654f6e58d1052576a9596ad7479b292474a2d79e21340b2","pre-Math live default-career data drifted");
...
assert.equal(meta.settings.muted,true,"persisted mute preference was not normalized");
assert.equal(meta.settings.characterLayout,"modern","legacy saves without Character layout must normalize to Modern");
assert.equal(defaults.settings.muted,false,"new careers must start with sound enabled");
...
assert.deepEqual([...meta.heirloomStorage],[],"legacy saves must normalize missing Heirloom Vault state");
assert.deepEqual(JSON.parse(JSON.stringify(meta.echoCrucible)),{learnedEffectIds:[],selectedEffectId:null,moonMetal:0},"legacy saves must normalize missing Echo Crucible state");
