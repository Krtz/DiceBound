...
async function main(){
  ...
  try{
    ...
    const actual=await page.evaluate(`(async()=>{
      ...
      {const before=restore('moon-held');progression.setPrestige({count:3});finish({name:'moon-held',kind:'moon',inspect:progression.prestigeInspect()},before);}
      {const before=restore('moon-random-stats');progression.setPrestige({count:1});const result=progression.prestigeDomainPurchase('five-random-stats');finish({name:'moon-random-stats',kind:'moon',result,inspect:progression.prestigeInspect()},before);}
      {const before=restore('moon-storage-chain');progression.setPrestige({count:8});const a=progression.prestigeDomainPurchase('heirloom-storage'),b=progression.prestigeDomainPurchase('heirloom-slot-i'),c=progression.prestigeDomainPurchase('heirloom-slot-ii');finish({name:'moon-storage-chain',kind:'moon',results:[a,b,c],inspect:progression.prestigeInspect()},before);}
      {const before=restore('moon-refund');progression.setPrestige({count:5});const a=progression.prestigeDomainPurchase('heirloom-storage'),b=progression.prestigeDomainPurchase('five-random-stats'),refund=progression.prestigeDomainRefund();finish({name:'moon-refund',kind:'moon',results:[a,b],refund,inspect:progression.prestigeInspect()},before);}
