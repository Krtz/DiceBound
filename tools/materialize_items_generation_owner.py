...
expected=['runtime/index.html','runtime/js/module-manifest.json','runtime/js/dicebound.js','runtime/build-info.json','runtime/build-manifest.json']
if sorted(changed)!=sorted(expected):raise SystemExit(f'unexpected Items generation migration diff: {changed}')
entries=[]
...
commit=post('/commits',{'message':'architecture: move Items generation ownership behind DiceboundItems','tree':tree['sha'],'parents':[parent]})
