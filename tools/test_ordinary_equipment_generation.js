...
const snapshots=[];
for(const rarity of Object.keys(rarityBudgets))for(const slot of equipment.createRegistry().slots){const code=`D15|${rarity}|${slot}|ranger|q8|fixture-${rarity}-${slot}`;const actual=equipment.generateOrdinaryFromSeedCode(code,deps),expected=legacyGenerateFromSeedCode(code),rolled=JSON.parse(JSON.stringify(actual));delete rolled.equipmentId;rolled.name=expected.name;assert.deepEqual(rolled,JSON.parse(JSON.stringify(expected)),`${rarity}/${slot} changed its ordinary rolled point budget or seeded RNG order`);const identity=equipment.identityForItem(actual);assert.ok(identity,`${rarity}/${slot} did not receive a stable equipment identity`);assert.equal(identity.slot,slot,`${rarity}/${slot} received an identity for another slot`);assert.deepEqual(Object.keys(actual).sort(),["affixTier","bonuses","elementPowerCost","equipmentId","icon","id","itemPower","name","prefix","rarity","seed","seedCode","slot","spentPower","suffix","suffixTier"].concat(slot==="weapon"&&actual.element?["element"]:[]).sort(),`${rarity}/${slot} shape drifted`);snapshots.push(rolled);}
