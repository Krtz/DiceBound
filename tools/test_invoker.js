...
classes.configureInvoker({
  ...
  playerAttack: async options => { attackCalls.push({ options: { ...(options || {}) }, multiplier: player._invokerAttackDamageMultiplier || 1 }); classes.invokerAfterPlayerAction(options?.postActionKind || "attack"); return options; },
...
(async () => {
  ...
  assert.equal(attackCalls[0].options.suppressEcho, true); assert.equal(attackCalls[0].options.postActionKind, "orb:blue"); assert.equal(attackCalls[0].multiplier, .85);
  ...
  assert.equal(attackCalls[1].options.suppressEcho, false); assert.equal(attackCalls[1].options.postActionKind, "orb:green"); assert.equal(attackCalls[1].multiplier, .85);
  await classes.invokerExortStrike();
  assert.deepEqual([...player._invoker.orbs], ["blue", "green", "red"]);
  assert.equal(attackCalls[2].options.suppressEcho, true); assert.equal(attackCalls[2].options.postActionKind, "orb:red"); assert.equal(attackCalls[2].multiplier, 1.20);
