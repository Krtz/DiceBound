...
async function main(){
  ...
  try{
    ...
    const reloadDeadline=Date.now()+20000;while(Date.now()<reloadDeadline){if(await page.evaluate("document.readyState==='complete'&&!!window.DiceboundCareerUi&&!!window.DiceboundProgression&&!!document.getElementById('campCareerBtn')"))break;await sleep(100);}
    await page.evaluate(`(()=>{window.__careerErrors=[];window.addEventListener('error',event=>window.__careerErrors.push(String(event.error?.stack||event.message||event.error||'window error')));return true;})()`);
