...
async function main(){
  ...
  try{
    ...
    if(campArt.count<2||!campArt.trophy.present||!campArt.tier||campArt.trophy.tier!==campArt.tier||!campArt.trophy.src.endsWith(`assets/camp/interactions/achievements/${campArt.tier}.png`)||campArt.trophy.aria!=='Achievements'||Math.abs(campArt.trophy.w-campArt.trophy.fw)>3||Math.abs(campArt.trophy.h-campArt.trophy.fh)>3)throw new Error(`Achievement tier art/hitbox regression: ${JSON.stringify(campArt)}`);
