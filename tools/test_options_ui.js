...
ui.configure({getSettings:()=>({muted:true,masterVolume:1.5,soundPack:"custom",floatingCombatNumbers:false,fastWheelSlots:true,fastWheelSlotsUnlocked:true}),nativeSaveSupported:()=>true});
const model=ui.sync();
assert.deepEqual(JSON.parse(JSON.stringify(model)),{owner:"ui/options",muted:true,volume:1,soundPack:"custom",floatingCombatNumbers:false,fastWheelSlots:true,fastWheelSlotsUnlocked:true,nativeSaveSupported:true},"Options settings view must normalize a read-only runtime snapshot");
