...
if sorted(changed)!=sorted(expected):raise SystemExit(f'unexpected Items operations migration diff: {changed}')
entries=[]
...
commit=post('/commits',{'message':'architecture: move Items operations ownership behind DiceboundItems','tree':tree['sha'],'parents':[parent]})
