...
(async()=>{
  ...
  try{
    ...
    assert.equal(gearDetail.modernTip,'',"Modern Character Gear must not fall back to the generic root data-tip path");
    assert.equal(gearDetail.slotKey,'hat',"rich Character Gear detail must use the rendered slot key");
    assert.equal(gearDetail.popVisible,true,"player-visible Gear detail popover did not open");
    assert.equal(gearDetail.popName,'Crown of the Road That Should Not Exist',"generic special-item names must recover canonical Artifact identity");
    assert.equal(gearDetail.popMeta,'MYTHICAL · HAT',"rich Gear detail must identify rarity and actual slot");
