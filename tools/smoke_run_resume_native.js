...
(async()=>{
  ...
  try{
    first=await launch(localAppData);console.log("Native pass 1 loaded through WebView2; writing deterministic checkpoint");
    const created=await first.evaluate(`(async()=>{window.DiceboundRng.seed('native-resume-seed');document.getElementById('campGoBtn').click();await new Promise(resolve=>setTimeout(resolve,240));const automaticSaved=!!window.DiceboundRunCheckpoint.load().checkpoint;const checkpoint=window.DiceboundRunResumeTest.snapshot();checkpoint.run.player.gold=654;checkpoint.run.player.position=Math.min(3,checkpoint.run.tiles.length-1);checkpoint.summary.gold=654;checkpoint.summary.tile=checkpoint.run.player.position+1;window.DiceboundRunCheckpoint.store(checkpoint);const expected=[window.DiceboundRng.random(),window.DiceboundRng.random(),window.DiceboundRng.random()];return {title:document.title,storage:window.DiceboundStorage.diagnostics(),checkpoint,expected,automaticSaved};})()`);
    assert.equal(created.title,"Dicebound: Beta v0.6.3.0");assert.equal(created.storage.backend,"wrapper-native");assert.equal(created.automaticSaved,true,"starting a stable native run did not autosave");
    ...
    const log=fs.readFileSync(path.join(localAppData,"Dicebound","logs","native-wrapper.log"),"utf8");assert.match(log,/WebView2 bootstrap mode=official-loader/);assert.ok((log.match(/Frontend ready handshake received for Beta 0\.6\.3\.0\./g)||[]).length>=2,"both native launches did not report frontend-ready");
