...
if sorted(changed)!=sorted(expected):
    raise SystemExit(f'unexpected Items facade seam diff: {changed}')
entries=[]
...
commit=post('/commits',{'message':'architecture: route first callers through DiceboundItems','tree':tree['sha'],'parents':[parent]})
