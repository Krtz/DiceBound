...
(async()=>{
  ...
  try{
    ...
    const byKind=Object.fromEntries(presentation.map(entry=>[entry.kind,entry]));const narrow=byKind.final.narrow,miniSize=narrow?96:120,finalSize=narrow?190:270,tieredHeight=narrow?Math.min(202,Math.max(128,Math.round(byKind.slime.viewportHeight*.24))):Math.min(342,Math.max(176,Math.round(byKind.slime.viewportHeight*.27)));assert.match(byKind.miniboss.hostClasses,/miniboss/);assert.match(byKind.final.hostClasses,/final-boss/);assert.deepEqual(byKind.miniboss.art,{width:miniSize,height:miniSize});assert.deepEqual(byKind.final.art,{width:finalSize,height:finalSize});assert.ok(byKind.final.art.width>byKind.final.player.width,`final boss portrait must be larger than the player portrait: ${JSON.stringify(byKind.final)}`);assert.match(byKind.slime.stageClasses,/db0636-tiered-enemy-stage/);assert.match(byKind.wolf.stageClasses,/db0636-tiered-enemy-stage/);assert.equal(byKind.slime.sprite.height,tieredHeight);assert.equal(byKind.wolf.sprite.height,tieredHeight);
