...
async function main(){
  ...
  try{
    ...
    const result=await page.evaluate(`(async()=>{
      ...
      ordinary?.click();await new Promise(resolve=>setTimeout(resolve,100));const soldAfterPurchase=document.querySelectorAll('#shopGrid .shop-item.sold').length;
      ...
      return {version:window.DiceboundVersion?.version,merchantHidden:document.getElementById('merchantOverlay')?.classList.contains('hidden'),shopItems:shop.length,comparisons:document.querySelectorAll('#shopGrid .shop-compare').length,comparisonText:document.querySelector('#shopGrid .shop-compare')?.textContent||'',rarityClass:rarityName?.className||'',sold:soldAfterPurchase,goldBefore,goldAfter:Number(document.getElementById('merchantGold')?.textContent||0),merchantState,refreshedFacePrimed:refreshedFace?.classList.contains('merchant-primed')||false,debug};
    })()`);
