...
assert.equal(registry.find(entry => entry.id === "nice")?.name, "Nice.");
assert.equal(registry.find(entry => entry.id === "nice")?.condition, "summonsCreated:69");
assert.equal(registry.find(entry => entry.id === "nice")?.secret, true);
assert.equal(registry.find(entry => entry.id === "nice")?.description, "You know what you did.");
assert.deepEqual(registry.find(entry => entry.id === "nice")?.hierarchy, {group:"secrets",subgroup:null,heroId:null});
