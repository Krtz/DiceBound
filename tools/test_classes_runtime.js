...
assert.match(monolith,/function performFixedCombatSlot\(slot,legacyAction\)\{/,'fixed compatibility slots must have one routing seam');
assert.doesNotMatch(monolith,/function performFixedCombatSlot[\\s\\S]{0,260}necromancer/i,'fixed-slot Combat action routing must remain class-agnostic');
assert.match(monolith,/const action=dbActionForFixedSlot\(slot\);/,'fixed slots must resolve semantic Combat actions by metadata rather than class-specific button branches');
assert.match(monolith,/return action\?dbExecuteCombatAction\(action\.id\):dbClasses\.performAction\(legacyAction\);/,'fixed slots must dispatch through the Combat action registry when a semantic action exists');
assert.match(monolith,/return action\?dbExecuteCombatAction\(action\.id\):dbClasses\.performAction\(legacyAction\);/,'fixed slots without a semantic action must preserve the released Classes action route');
assert.match(monolith,/replaceCombatButton\("attackBtn",\(\)=>performFixedCombatSlot\("attack","attack"\)\)/);
...
assert.match(monolith,/replaceCombatButton\("ultimateBtn",\(\)=>performFixedCombatSlot\("ultimate","ultimate"\)\)/);
assert.match(monolith,/specialAttackBtn\.addEventListener\("click",\(\)=>performFixedCombatSlot\("special","special"\)\)/);
assert.match(monolith,/dbCombatActionRegistry=dbCombatActionRegistryOwner\.createRegistry\(\)/,'Combat action registry must be composed once');
assert.match(monolith,/id:'class-necromancer'[\s\S]*getActions:\(\)=>dbClasses\.necromancerActionDescriptors\(\)/,'Necromancer action descriptors must register as a semantic provider');
