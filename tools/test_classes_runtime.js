...
assert.match(monolith,/if\(classIdentityActive\("necromancer"\)\)\{/,'Necromancer must be the first native registry-backed fixed-slot consumer');
...
assert.match(monolith,/if\(action\)return dbExecuteCombatAction\(action\.id\);/,'Necromancer fixed slots must dispatch through the Combat action registry');
assert.match(monolith,/return dbClasses\.performAction\(legacyAction\);/,'all non-migrated classes must preserve the released Classes action route');
