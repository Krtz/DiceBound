...
assert.match(monolith,/function showEnd\(victory\)\{dbRunClearCheckpoint\(\);/,"terminal end flow must clear the active-run checkpoint directly");
assert.doesNotMatch(monolith,/const dbRunShowEndBase=showEnd;showEnd=function/,"retired showEnd checkpoint wrapper returned to the monolith");
assert.match(monolith,/DiceboundCamp\.configureShell\(\{scheduleRunCheckpoint:\(\)=>dbRunScheduleCheckpoint\(\),clearCheckpoint:\(\)=>dbRunClearCheckpoint\(\),refreshRunControls:\(\)=>dbRunRefreshControls\(\)\}\);/,"Run checkpoint clearing must remain injected into the authoritative Camp shell boundary");
...
assert.match(monolith,/petTurn:\(\.\.\.args\)=>dbCombat\.petTurn\(\.\.\.args\)/,"Pet Turn consumers must route directly through the Combat facade");
assert.doesNotMatch(monolith,/function\s+petTurn\s*\(/,"retired standalone petTurn compatibility seam returned to the monolith");
