...
def main()->int:
    ...
    # DiceboundPowerupRegistry, so the old mutation/copy pass is both ineffective
    # (the registry view is read-only) and dangerous when its dead table vanishes.
    # Preserve the regression surface by deriving it from canonical metadata.
    ...
    # canonical asset manifest only for secret bosses that are not board guardians.
    text=replace_once(
        text,
    ...
    # The deleted compatibility declarations must stay deleted.  A direct
    # reference here would recreate the exact startup crash this repair targets.
    ...
    print("0.6.7.0 startup-owner repair: retired db0512 reward replay and db060 guardian alias")
    return 0
