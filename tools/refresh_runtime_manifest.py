...
def main():
    ...
    payload=sorted(
        (p for p in runtime.rglob('*')
         ...
         and p.suffix.lower() in RUNTIME_EXTENSIONS),
    ...
    for p in payload:
        ...
        h.update(b'\0')
        h.update(sha256_runtime_file(p).encode())
        h.update(b'\n')
    digest=h.hexdigest()
    ...
    info={
      ...
      'saveSchemaVersion':2,
      'sourceHash':digest,
      'timestampPolicy':'none; materialized identity is content-derived from runtime-loadable HTML/CSS/JS/image/audio payloads; HTML/CSS/JS hashes normalize CRLF to LF',
      'version':version,
    ...
    manifest={
      ...
      'files':{rel:sha256_runtime_file(runtime/rel) for rel in core},
      'reproducible':True,
      'validation':'python tools/validate_asset_architecture.py',
      'hashPolicy':'browserContentHash authenticates every runtime-loadable HTML/CSS/JS/image/audio payload using canonical LF source-text hashes; files carries per-file hashes for build-info and core runtime entrypoints/scripts.'
    }
