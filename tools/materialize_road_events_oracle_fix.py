...
if changed!=['tools/test_road_events_oracle.js']:
    raise SystemExit(f'unexpected oracle-fix diff: {changed}')
entries=[]
...
commit=post('/commits',{'message':'test: decouple Road Events baseline from release version','tree':tree['sha'],'parents':[parent]})
