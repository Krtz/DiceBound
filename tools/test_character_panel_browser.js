...
async function snapshot(page){return page.evaluate(`(()=>{const card=document.getElementById('characterCard'),stats=document.getElementById('characterStatsPanel'),gear=document.getElementById('characterGearPanel'),grid=document.getElementById('equipmentGrid'),board=document.getElementById('boardWrap'),controls=document.querySelector('.road-controls'),panel=document.querySelector('.game-panel'),rect=card?.getBoundingClientRect(),gridRect=grid?.getBoundingClientRect(),boardRect=board?.getBoundingClientRect(),controlsRect=controls?.getBoundingClientRect(),panelRect=panel?.getBoundingClientRect();return {card:{width:rect?.width||0,height:rect?.height||0,left:rect?.left||0,right:rect?.right||0,top:rect?.top||0,bottom:rect?.bottom||0},statsHidden:!!stats?.hidden,gearHidden:!!gear?.hidden,gearDisplay:gear?getComputedStyle(gear).display:null,grid:{width:gridRect?.width||0,height:gridRect?.height||0},slots:grid?.querySelectorAll('.character-gear-slot').length||0,activeTab:document.querySelector('#characterTabs [data-character-tab].active')?.dataset.characterTab||null,duplicateEquipmentCards:document.querySelectorAll('.equipment-card').length,playFlow:document.body.dataset.playFlow||'',hudFlow:document.body.dataset.hudFlow||'',board:{left:boardRect?.left||0,right:boardRect?.right||0,top:boardRect?.top||0,bottom:boardRect?.bottom||0,width:boardRect?.width||0,height:boardRect?.height||0},controls:{left:controlsRect?.left||0,right:controlsRect?.right||0,top:controlsRect?.top||0,bottom:controlsRect?.bottom||0,width:controlsRect?.width||0,height:controlsRect?.height||0},panel:{left:panelRect?.left||0,right:panelRect?.right||0,top:panelRect?.top||0,bottom:panelRect?.bottom||0,width:panelRect?.width||0,height:panelRect?.height||0}};})()`);}
...
async function main(){
 ...
 try{
   ...
   if(Math.abs(widePlay.board.width-widePlay.board.height)>2||widePlay.board.width<720)throw new Error(`Roomy desktop Board did not stay square and materially grow: ${JSON.stringify(widePlay)}`);
   ...
   console.log("Character Edge PASS: spatial Modern Gear, authoritative tooltip details, wide Board side-controls, live resize, Classic fallback and restore");
 }finally{
