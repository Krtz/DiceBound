...
async function main(){
 ...
 try{
   ...
   for(const [width,height] of sizes){
     ...
     if(state.characterPetOverlap>1)throw new Error(`Legacy Companion overlaps Character at ${width}x${height}: ${JSON.stringify(state)}`);
     if(state.legacyCompanionLayout)throw new Error(`Retired data-sidebar-companion layout state returned at ${width}x${height}: ${JSON.stringify(state)}`);
   ...
   if(resized.characterPetOverlap>1||resized.legacyCompanionLayout)throw new Error(`Live resize let Legacy Companion collide with Character: ${JSON.stringify(resized)}`);
