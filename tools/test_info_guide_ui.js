...
for(const adapter of ["renderInfo","activateInfoTab","openInfo"])assert.ok(!new RegExp(`^\\s*${adapter}\\s*=`,"m").test(monolith),`retired ${adapter} reassignment remains`);
...
assert.doesNotMatch(source,/data-info-tab="stats"/,"Career Stats must not remain duplicated inside Info");
assert.doesNotMatch(source,/lifetimeModel/,"Career lifetime aggregation must not remain owned by Info");
