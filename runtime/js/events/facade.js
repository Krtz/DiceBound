 ...
 * to extracted internal owners where available and otherwise to the remaining
 * authoritative lifecycle implementations under the frozen 0.6.6.25 oracle.
 */
(function(){
  'use strict';
  ...
  function configure(nextRuntime={}){
    const {treasure:treasureRuntime,...handlers}=nextRuntime;
    runtime={...runtime,...handlers};
    if(treasureRuntime){
      const owner=window.DiceboundRoadEventTreasure;
      if(!owner?.configure)throw new Error('DiceboundRoadEvents requires DiceboundRoadEventTreasure before configuring Treasure.');
      treasure=owner.configure(treasureRuntime);
    }
    return api;
  }
  function openSlot(...args){return requireHandler('openSlot')(...args);}
  function openWheel(...args){return requireHandler('openWheel')(...args);}
  function openTreasure(...args){return treasure?.open?treasure.open(...args):requireHandler('openTreasure')(...args);}
  ...
  function inspect(){
    return Object.freeze({
      ...
      configured:Object.freeze({
        ...
        treasure:!!treasure?.open||typeof runtime.openTreasure==='function',
      ...
      internals:Object.freeze({treasure:treasure?.owner||null})
    });
  }
