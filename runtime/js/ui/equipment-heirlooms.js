  ...
  function renderEndGear(){
    ...
    if(current.storageUnlocked){
      ...
      items.forEach(item=>{
        ...
        button.innerHTML=`<strong>${isStored?'✓ STORED · ':''}${itemNameMarkup(item,'')}</strong><span>${escapeHtml(rarity(item).label||item.rarity||'Unknown')} · ${escapeHtml(label(item.slot))} · ${escapeHtml(bonuses(item))}</span>`;
    ...
    eligible.forEach(item=>{
      ...
      button.innerHTML=`<strong>${itemNameMarkup(item,'')} </strong><span>${escapeHtml(label(item.slot))} · ${escapeHtml(bonuses(item))}</span>`;
