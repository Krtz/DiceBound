...
(function(root){
  ...
  function rarity(item){return runtime.getRarityInfo?.(item?.rarity)||root.DiceboundRarities?.infoFor?.(item?.rarity)||{label:item?.rarity||'Unknown'};}
  function rarityColor(itemOrId){const id=typeof itemOrId==="string"?itemOrId:itemOrId?.rarity;return root.DiceboundRarities?.colorFor?.(id)||rarity({rarity:id}).color||"#ffffff";}
  ...
  function vaultSlotMarkup(slot,activeBySlot){
    const item=activeBySlot.get(slot),slotId=escapeHtml(slot),detail=escapeHtml(itemDetail(item,slot)),rarityStyle=item?` style="--db-rarity-color:${escapeHtml(rarityColor(item))}"`:"";
    return `<div class="vault-paper-slot vault-slot-${slotId} ${item?.rarity||'empty'}" data-vault-slot="${slotId}" title="${detail}" aria-label="${detail}"${rarityStyle}>${item?gearIconMarkup(item,'db-vault-slot-art'):`<span class="vault-paper-slot-label">${escapeHtml(label(slot))}</span>`}</div>`;
  }
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      .character-gear-slot.empty{border-color:rgba(255,255,255,.10)!important}.character-gear-slot:not(.empty),.vault-paper-slot:not(.empty){border-color:var(--db-rarity-color,#fff)!important;box-shadow:0 0 10px rgba(0,0,0,.28),inset 0 0 12px rgba(255,255,255,.035)!important}
  ...
  function showDetailPopover(target){
    ...
    pop.style.setProperty('--db-rarity-color',rarityColor(item));
  ...
  function rarityNameMarkup(item){
    const rarityId=escapeHtml(String(item?.rarity||"common").toLowerCase()),color=escapeHtml(rarityColor(item));
    return `<span class="db-rarity-name db-rarity-${rarityId}" style="--db-rarity-color:${color}">${escapeHtml(displayName(item))}</span>`;
  }
  ...
  function renderEquipment(){
    ...
    if(grid){
      ...
      slots().forEach(slot=>{
        ...
        if(item)entry.style?.setProperty('--db-rarity-color',rarityColor(item));else entry.style?.removeProperty('--db-rarity-color');
  ...
  function renderEndGear(){
    ...
    if(!grid)return null;
    grid.replaceChildren();
    const eligibleItems=items.filter(item=>runtime.isHeirloomEligible?.(item)!==false);
    if(current.storageUnlocked){
      if(status)status.innerHTML=`Heirloom Vault: <strong>${(current.storage||[]).length}/${Math.max(0,Number(current.storageCapacity)||0)}</strong>. Click eligible surviving run gear to store it. Choose the active next-run loadout from the Campsite chest.`;
      if(!eligibleItems.length){grid.innerHTML='<div class=\"hint\">No heirloom-eligible equipment survived this run. Stored heirlooms remain safe.</div>';renderEndStorageManager();return Object.freeze({owner:OWNER,storage:true,items:0});}
      ...
      renderEndStorageManager();return Object.freeze({owner:OWNER,storage:true,items:eligibleItems.length});
    }
    const eligible=eligibleItems,bound=current.heirlooms||[],capacity=Math.max(0,Number(current.activeCapacity)||0);
