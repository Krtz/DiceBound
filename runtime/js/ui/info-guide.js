...
(function(root){
  ...
  function guideHtml(){return [
    ...
    detail('legacy','Legacy, Talents & Prestige','<p>Run distance and banked gold become Legacy XP. Legacy levels grant talent points. Talents purchased during a run activate on the <b>next</b> run.</p><p>Prestige converts every 9 total talent points into unspent Prestige Points, then resets Legacy level and the talent tree. Every lifetime Prestige Point permanently adds <b>+5% Legacy XP per run</b>; each currently unspent point also grants one held stat point. The Prestige Moon can buy permanent Heirloom Loadout and Vault Expansion ranks or convert refundable points into persisted five-stat random bundles. <b>The Heirloom Vault and everything stored inside it survive automatically.</b></p>'),
