(() => {
  ...
  const manifest=Object.freeze({version:20,
    enemies:Object.freeze({
      ...
      orc:normalEnemy("orc","Orc"),cultist:normalEnemy("cultist","Cultist"),wraith:Object.freeze({battleByBoard:Object.freeze(Object.fromEntries([1,2,3,4,5,6].map(board=>[String(board),`${paths.normalEnemyBattle}/wraith-board-${board}.png`]))),boardMarker:`${paths.normalEnemyMarkers}/wraith.png`,alt:"Wraith",matte:"dark"}),
  ...
  const resolveEnemyBattleArt=(name,level=1)=>{const m=matchEnemy(name);if(!m)return null;const e=manifest.enemies[m.key],board=normalizeBattleBoard(level),src=e.battleByBoard?.[String(board)]||null;return src?Object.freeze({key:m.key,src,alt:e.alt||String(name),board,...(e.matte?{matte:e.matte}:{})}):null};
