...
(() => {
  ...
  function configure(deps={}){
    ...
    function initialize(classId){
      ...
      deps.setCombatKind(null);player.powerupRerolls=deps.gameplayTalentRank("fortune_powerup_rerolls");player.rangerMarkMax=3+deps.gameplayTalentRank("ranger_deep_marks");player.monkComboMax=5+deps.gameplayTalentRank("monk_flow_ceiling");player.fighterCounterStacks=0;player.fighterCounterMax=1+Math.min(1,deps.gameplayTalentRank("fighter_counter_reserve"));player.fighterCounterReady=false;player.turtleCrushReady=false;player.turtleGuardChain=0;delete player.turtleConsecutiveGuards;player.turtleGuardMax=5;player.secondSun=false;player.secondSunUsedBoards={};player.radiationDefenseLost=0;player.alchemistBrewCounter=0;player.alchemistBrewNeed=3;player.alchemistFlaskBonus=0;player.alchemistElementChance=0;player.alchemistFreeFlask=0;player._activePetBonusId=null;if(deps.classIdentityActive("alchemist")){player.potions+=2;if(player.classId!=="alchemist")player.potionPower+=.50;}deps.syncActivePetBonus(true);
