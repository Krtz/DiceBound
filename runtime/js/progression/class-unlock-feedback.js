(() => {
  ...
  function requirementPhrase(requirement={}){
    switch(requirement.type){
      ...
      case "boardClears":{
        const requirements=Array.isArray(requirement.requirements)?requirement.requirements:[];
        if(requirements.length===2&&requirements[0].board===requirements[1].board)return `You cleared Board ${formatNumber(requirements[0].board)} with both ${className(requirements[0].classId)} and ${className(requirements[1].classId)}.`;
        return requirements.map(requirementPhrase).join(" ");
      }
  ...
  function joinRequirements(requirements=[]){
    ...
    return cleaned.length===2?`You ${cleaned[0]} and ${cleaned[1]}.`:`You ${cleaned.slice(0,-1).join(", ")}, and ${cleaned[cleaned.length-1]}.`;
  }
  ...
  function ensureOverlay(){
    ...
    ensureStyle();overlay=document.createElement("div");overlay.className="overlay hidden";overlay.id="classUnlockRevealOverlay";overlay.setAttribute("role","dialog");overlay.setAttribute("aria-modal","true");overlay.setAttribute("aria-labelledby","classUnlockRevealName");
    overlay.innerHTML=`<div class="modal wide class-unlock-reveal-modal"><div class="class-unlock-reveal-kicker">NEW CLASS UNLOCKED</div><div class="class-unlock-reveal-art-wrap"><img class="class-unlock-reveal-art" id="classUnlockRevealArt" alt=""></div><h2 id="classUnlockRevealName">New class</h2><p class="class-unlock-reveal-identity" id="classUnlockRevealIdentity"></p><div class="class-unlock-reveal-copy"><div class="class-unlock-reveal-card"><strong>How it plays</strong><span id="classUnlockRevealPlay"></span></div><div class="class-unlock-reveal-card"><strong>Why you unlocked it</strong><span id="classUnlockRevealWhy"></span></div></div><div class="class-unlock-reveal-progress" id="classUnlockRevealProgress"></div><button class="main-btn" id="classUnlockRevealContinue" style="width:100%">Got it</button></div>`;
    document.body.appendChild(overlay);
    document.getElementById("classUnlockRevealContinue").addEventListener("click",()=>{if(activeId)acknowledge(activeId);overlay.classList.add("hidden");setTimeout(showNextReveal,80);});
    return overlay;
  }
  ...
  function onClassUnlocked(classId){
    const queued=enqueue(classId);
    if(queued&&typeof document!=="undefined"&&campVisible())setTimeout(showNextReveal,120);
    return Object.freeze({queued,toast:toastFor(classId)});
  }
  function startCampObserver(){
    loadState();
    if(typeof document==="undefined"||typeof MutationObserver==="undefined")return;
    const camp=document.getElementById("startOverlay");
    if(camp)new MutationObserver(()=>{if(campVisible())setTimeout(showNextReveal,120);}).observe(camp,{attributes:true,attributeFilter:["class"]});
    if(campVisible())setTimeout(showNextReveal,120);
  }

  window.DiceboundClassUnlockFeedback=Object.freeze({apiVersion:2,reasonFor,toastFor,revealFor,requirementPhrase,enqueue,acknowledge,onClassUnlocked,showNextReveal,state:()=>({pending:[...state.pending],acknowledged:[...state.acknowledged],activeId})});
  startCampObserver();
})();
