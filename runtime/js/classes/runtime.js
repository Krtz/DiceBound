(() => {
  ...
  function initIdentitySupport(id){
    ...
    if(mechanics.has('mana')){
      const desiredMax=id==='summoner'?120:id==='necromancer'?110:100,desiredStart=id==='summoner'?35:id==='necromancer'?30:25;
    ...
    if(mechanics.has('allies')){player.maxActiveAllies=Math.max(0,Number(player.maxActiveAllies??2)||0);player.graveCount=Math.max(0,Number(player.graveCount)||0);player.graveCountThreshold=Math.max(2,Number(player.graveCountThreshold)||5);}
