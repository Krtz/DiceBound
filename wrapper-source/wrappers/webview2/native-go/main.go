...
const (
    appTitle       = "Dicebound: Beta v0.5.8"
...
func fatal(err error) { logf("FATAL: %v",err); messageBox("Dicebound Beta 0.5.8", err.Error()+"\n\nNative wrapper log:\n"+logPath); os.Exit(1) }
...
func allStorageKeys(saveDir string) []string {
    known:=[]string{"dicebound.save.primary","dicebound.save.backup","dicebound.save.backup.2","dicebound.save.backup.3","dicebound.save.backup.4","dicebound.save.backup.5"};out:=[]string{}
...
func watchFrontendReady(){
    select{
    case <-frontendReady: logf("Frontend ready handshake received for Beta 0.5.8.")
...
func main(){
    ...
    logf("Starting Dicebound Beta 0.5.8 native WebView2 wrapper. payload=%s repairAttempted=%v",buildKey,repairAttempted);logf("data=%s saves=%s runtime-cache=%s game=%s webview2=%s",dataDir,saveDir,runtimeCacheDir,gameDir,userDataDir)
    ...
    base,err:=startServer(gameDir,saveDir);if err!=nil{fatal(fmt.Errorf("could not start local Dicebound host: %w",err))};serverBase=base;initURL=base+"/index.html?diceboundNative=1&v=0.5.8&build="+buildKey;logf("Serving exact browser payload at %s",base)
