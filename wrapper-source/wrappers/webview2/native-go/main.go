...
const (
    appTitle       = "Dicebound: Beta v0.6.4.16"
...
func watchFrontendReady(){
    select{
    case <-frontendReady: logf("Frontend ready handshake received for Beta 0.6.4.16.")
...
func main(){
    ...
    logf("Starting Dicebound Beta 0.6.4.16 native WebView2 wrapper. payload=%s repairAttempted=%v",buildKey,repairAttempted);logf("data=%s saves=%s runtime-cache=%s game=%s webview2=%s",dataDir,saveDir,runtimeCacheDir,gameDir,userDataDir)
    ...
    base,err:=startServer(gameDir,saveDir);if err!=nil{fatal(fmt.Errorf("could not start local Dicebound host: %w",err))};serverBase=base;initURL=base+"/index.html?diceboundNative=1&v=0.6.4.16&build="+buildKey;logf("Serving exact browser payload at %s",base)
