...
`tools/build_launcher.py` embeds that DLL into the single Dicebound EXE. At startup the wrapper extracts the exact loader into `%LOCALAPPDATA%\Dicebound\wrapper\WebView2Loader.dll` and calls the public `CreateCoreWebView2EnvironmentWithOptions` export.
